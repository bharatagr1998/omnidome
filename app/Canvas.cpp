...
//                    SLOT(selectCanvasType(QString)));
            connect(ui_->boxCanvasSelect, &QComboBox::currentTextChanged,
                    this, [this](const QString &text){
                        selectCanvasType(text.toUtf8()); // convert QString -> QByteArray
                    });
        }
        ...
        void Canvas::dataToFrontend()
        {
            if (!dataModel()->canvas())
            {
              ...
              //  selectCanvasType("HalfDome"); Below is Ai generated code
//                selectCanvasType(QByteArray("HalfDome"));
                selectCanvasType(QByteArrayLiteral("HalfDome"));
            }
